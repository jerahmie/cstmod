...
def check_for_fields(results_dir, results_pattern):
    ...
    result_name_slices = re.match(r'^([a-zA-Z0-9\(\)\-=. ]*_)([0-9]*)(,[0-9]*)(.m3d|.m3t|_m3d.rex|_m3d_sct.rex)$', os.path.basename(cst_internal_results_sorted[-1])).groups()
    ...
    if max_result == len(cst_internal_results_sorted):
        for result in range(1,max_result+1):
            ...
            if os.path.basename(check_file) not in cst_internal_results_sorted:
                missing_results.append(check_file)

    return len(missing_results), nresults, missing_results
...
if __name__ == "__main__":
    ...
    n_missing, n_total, missing_results = check_for_fields(cst_dir, results_pattern)
    print("Number of results expected: "  + str(n_total) + " / Number fields missing: " + str(n_missing))
